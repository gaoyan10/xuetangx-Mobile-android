...
public class AllCourseAnalyzer implements Analyzer {
			...
			try {
			    ...
			    if (obj.getString("status").equals("success")) {
			    	...
			    	for (int i = 0; i < list.length(); i ++) {
						...
						map.put("display_coursenum", item.getString("display_coursenum"));
						map.put("start", item.getString("start"));
						map.put("advertised_start", item.getString("advertised_start"));
						map.put("short_description", item.getString("short_description"));
						map.put("course_image_url", item.getString("course_image_url"));
	...
	public ResponseMessage connect() {
		...
		ResponseMessage msg = con.httpGet(ConstantUtils.URL + ConstantUtils.GET_ALL_COURSES, params);
		return msg;
	}
